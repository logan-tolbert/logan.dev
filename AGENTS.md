...
## Workflow
- Work on `dev`. `main` is deploy-only: a push to `main` triggers the GitHub Actions workflow at `.github/workflows/deploy.yml`, which copies `assets/`, `pages/`, `index.html`, `CNAME` into `dist/`, minifies the 4 CSS files (clean-css), optimizes images (imagemin), runs a **non-failing** lychee link check, and deploys to GitHub Pages (`www.logan-tolbert.com` per `CNAME`).
...
- **CSS:** `assets/styles/base.css` holds design tokens (CSS custom properties) plus shared components (navbar, buttons, cards, footer). Each page has its own stylesheet (`index.css`, `about.css`, `contact.css`) linked alongside `base.css`. Add a page-specific file when adding a page; keep shared styles in `base.css`.
